...
def clean_insee_file(insee_file):
    ...
    firstnames_clean = firstnames_clean.rename(
        columns={"preusuel_clean": "firstname", 1: "proba_male", 2: "proba_female"}
    )

    firstnames_clean["total"] = firstnames_clean.proba_male + firstnames_clean.proba_female
    firstnames_clean["proba_male"] = round(
        firstnames_clean.proba_male.div(firstnames_clean.total).mul(100), 2
    )
    ...
    return firstnames_clean[["firstname", "firstname_length", "proba_male", "proba_female", "total"]].sort_values(by=["firstname_length", "total"], ascending=False).reset_index(drop=True)
